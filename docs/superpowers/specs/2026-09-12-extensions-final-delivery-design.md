...
### Plan D.3 adjustments (2026-09-13)

- **Result provenance rule.** Plan D.1 added `literature` to the source packages, which changed `source_hash` and made every earlier result directory look foreign to the report generators. A result directory is current when its manifest is complete and either its `source_hash` equals the current one, or it records a clean commit that is an ancestor of `HEAD` and `git diff --quiet --diff-filter=a <commit> -- src/models src/baselines src/bounds src/optimization src/literature` succeeds (added files are ignored). `src/runner` only schedules and records tasks, and the `experiments` level checks it end to end. Under this rule every result directory the report reads is current except `results/phase1/`, produced at `4433e38` before sub-project B changed `src/models` and `src/bounds/solve.py`; `results/phase2/pilot/` (the sub-project C pilot) also fails and is not read by the report.
- **Phase 1 refresh.** Rerunning Phase 1 at `f36ef23` (3725 tasks, 12 workers, 17 min) reproduced `method_realizations.csv` (3600 rows), `bounds.csv` (3600 rows) and `milp_crosscheck.csv` (5 rows) on every non-runtime column. The refreshed results replace the committed ones; in the report data only LP and MILP runtimes change.
- **Subset semantics.** `--subset` keeps the rows of one topology but counts only the scenarios an experiment runs (its split and replicates): `phase2_pilot` and `tran_pilot.py` use the development split and are skipped, the MILP cross-check is dropped, and a comparison fails when the rerun has no rows but the reference file has some. The first prototype run passed `phase2_pilot` with 0 matching rows against 720 reference rows, which motivated both rules; `phase2_budget` and `phase2_design` compute no bounds, so their header-only `bounds.csv` files still match.
- **Measured checks.** On the prototype, `tables` passed 41 checks in 5 s, `scenarios` passed 52 checks in 18 s, and `experiments --subset Agis` passed in 52 min with 12 workers (372, 432, 30, 495, 3, 192 and 195 tasks for `phase1`, `phase2_main`, `phase2_budget`, `phase2_sensitivity`, `phase2_design`, `phase2_literature`, `phase2_case_study`), every non-runtime column matching the committed rows. That run predates the split filter; with the filter, `phase2_pilot` was rerun alone and reported skipped, giving 16 checks.
- **Macro prefix.** The literature and case-study macros share the prefix `\ResultExt` instead of `\ResultLit` and `\ResultCase`, because `ext_results.tex` also carries the pilot, extraction, trace and compute values.
- **Quality–runtime figure.** `fig:quality-runtime` is a separate figure with the budget curves and the TRAN points, so the budget figure of sub-project C is unchanged.
- **Cross-reference words.** `cleveref` printed English "and" and "appendix" in the Vietnamese report (the introduction already did before this plan). The preamble now defines Vietnamese conjunctions and the appendix name "Phụ lục", and the compile check fails on these English words as well as on any warning of the final pass and on `??`.
- **RescueNet record.** The Crossref record of DOI 10.1038/s41597-023-02799-4 gives volume 10, issue 1, article 913; the entry gains `number = {1}`, and the "chưa được đối chiếu" note is replaced by that check.
- **Recorded compute.** The committed experiments record 74.3 h of task time in total (`docs/report/data/ext_compute.csv`, `tab:compute`); the prototype showed 74.0 h, and the difference is the Phase 1 task time of the refresh executed at `8428d0e` (3.3 h against 3.0 h, 18 min of wall time), whose non-runtime columns again matched the committed rows.
