...
Project code runs in the uv-managed `.venv` (commit `8876feb`); `scipy>=1.14,<2` is its only new dependency. LaTeX work follows the project skill `latex-document-skill`: tables and figures read CSV files at compile time with `csvsimple` and `pgfplots`, the patterns in the skill's `references/tables-and-images.md` (Tables from CSV Data) and `references/charts-and-graphs.md`, and the report is built with the skill's `compile_latex.sh`. Dependencies of skill scripts stay installed globally by the user and are not added to `.venv`.
...
results/phase1/ ──► experiments/report_tables.py ──► docs/report/data/*.csv ──► csvsimple / pgfplots
...
- **Configs and tables:** `v0-bh50.yaml` differs from `v0.yaml` only in the two fields; the generated `params.csv` contains every configured value, and the semicolon-separated report CSVs parse back to the expected rows.
...
- Report tables and the runtime figure read CSV at compile time through the skill's `csvsimple` and `pgfplots` patterns (Section 10).
- Decimal comma in the report (Section 10).
- Skill-script dependencies stay global; `.venv` holds project dependencies only (Section 3.3).
